...
export class ErrorService {
  ...
  getErrorMessage(ms, userForm) {
    ...
    if (ms == 'password') {
      ...
      if (userForm.get('password').hasError('pattern'))
        return 'Must have one letter, and one number';
      if (
        userForm.get('password').value != userForm.get('confirmPassword').value
      )
    ...
    if (ms == 'confirmPassword') {
      ...
      if (userForm.get('confirmPassword').hasError('minlength'))
        return 'Must be at least 8 characters long';
      if (userForm.get('confirmPassword').hasError('pattern'))
        return 'Must have one letter, and one number';
      if (
        userForm.get('password').value != userForm.get('confirmPassword').value
      )
    ...
    if (ms == 'gender')
      return userForm.get('gender').hasError('required')
        ? this.requiredString
        : '';
    if (ms == 'role') {
      if (userForm.get('role').hasError('required')) return this.requiredString;
      return userForm.get('role').hasError('maxlength')
    ...
    if (ms == 'phoneNumber') {
      if (userForm.get('phoneNumber').hasError('required'))
        return this.requiredString;
      if (userForm.get('phoneNumber').hasError('minlength'))
        return 'must have be at least 10 digit to 14.';
      if (userForm.get('phoneNumber').hasError('maxlength'))
        return 'must have be at least 10 digit to 14.';
    }

    if (ms == 'address1') {
      if (userForm.get('address1').hasError('required'))
        return this.requiredString;
      return userForm.get('address1').hasError('maxlength')
        ? 'Address must be least than 200 characters'
        : '';
    }
    if (ms == 'address2') {
      if (userForm.get('address2').hasError('required'))
        return this.requiredString;
      return userForm.get('address2').hasError('maxlength')
        ? 'Address must be least than 200 characters'
        : '';
    }
    ...
    if (ms == 'state') {
      if (userForm.get('state').hasError('required'))
        return this.requiredString;
      return userForm.get('state').hasError('maxlength')
        ? 'State must be less than 50 characters.'
        : '';
    }
    if (ms == 'city') {
      if (userForm.get('city').hasError('required')) return this.requiredString;
      return userForm.get('city').hasError('maxlength')
        ? 'city must be less than 50 characters'
        : '';
    }
    if (ms == 'zipcode')
      return userForm.get('zipcode').hasError('required')
        ? this.requiredString
        : '';
    if (ms == 'dateOfBirth')
      return userForm.get('dateOfBirth').hasError('required')
        ? this.requiredString
        : '';
  }
  ...
  checkingOnline() {
    if (!navigator.onLine) {
      return 'No Internet Conntection';
    }
  }
  ...
  getServerMessage(error: HttpErrorResponse): string {
    ...
    if (!!msg) return msg + ' : ' + error.error.ExceptionMessage;
